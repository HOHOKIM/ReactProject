...
//유투브 끌고오기
// 영화 비디오 가져오기
export const getMovieVideos = async (movieId: number): Promise<TMDBVideoResult[]> => {
...
export function getMovieDetails2(id: number): Promise<TMDBTVResult> {
  return fetch(`${BASE_PATH}/tv/${id}?api_key=${API_KEY}&language=ko-KR`).then(response => response.json());
}
...
export function getPersonDetails(id: number): Promise<TMDBPersonResult> {
  return fetch(`${BASE_PATH}/person/${id}?api_key=${API_KEY}&language=ko-KR`).then(response => response.json());
}

//등장인물 async
export const getCast = async (id: number, mediaType: 'movie' | 'tv'): Promise<ICast[]> => {
  const response = await axios.get<IGetCastResult>(`${BASE_PATH}/${mediaType}/${id}/credits`, {
    params: {
      api_key: API_KEY,
      language:'ko-KR'
    },
