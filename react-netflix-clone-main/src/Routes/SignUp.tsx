...
function SignUp() {
  ...
  const checkPhonenumber = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    console.log("핸드폰번호 유효성 검사 :: ", isValid);
    if (isValid) {
      console.log("통과");
    } else {
      setPhoneError("번호 형식 맞추라");
      return;
    }
  };
  ...
  const checkPassword = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    const regExp = /^(?=.*\d)(?=.*[a-zA-Z])(?=.*[\W_])[0-9a-zA-Z\W_]{8,20}$/;
  ...
  const checkBirth = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    console.log("생년월일 유효성 검사 :: ", isValid);
    if (isValid) {
      console.log("통과");
    } else {
      setBirthError("생년월일다시");
      return;
    }
  };
  // 비번 맞는지 확인
  ...
  const handleSignUp = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (nameError || EmailError || PwdError || PhoneError || BirthError) {
    ...
    <Container>
      ...
      <Form onSubmit={handleSignUp}>
        ...
        <InputWrapper>
          ...
          />
          <FloatingLabel>
            비밀번호(8자이상 영문자와 숫자, 특수문자 조합)
          </FloatingLabel>
        </InputWrapper>
        {PwdError && (
          <div style={{ color: "red", marginTop: "10px" }}>{PwdError}</div>
        )}
        {/* ----------------------------------- 비밀번호 확인 -----------------------------------*/}
        <InputWrapper>
          <InputContainer>
        ...
        {PhoneError && (
          <div style={{ color: "red", marginTop: "10px" }}>{PhoneError}</div>
        )}
        {/* ----------------------------------- 생년월일 -----------------------------------*/}
        <InputWrapper>
          <InputField
        ...
        {BirthError && (
          <div style={{ color: "red", marginTop: "10px" }}>{BirthError}</div>
        )}
        {/* ----------------------------------- 주소 -----------------------------------*/}
        <InputWrapper>
          <InputContainer>
