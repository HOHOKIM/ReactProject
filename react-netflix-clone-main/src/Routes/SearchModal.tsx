...
import { TMDBMovieResult, TMDBTVResult, TMDBPersonResult, TMDBVideoResult, ICast } from "../api";
...
export const DetailModal: React.FC<{
  ...
  const renderReleaseDate = () => {
    if ("release_date" in item) {
      return <Rating><p>개봉일: {item.release_date}</p></Rating>;
    }
  ...
  const renderRating = () => {
    if ("vote_average" in item) {
      return (
        <Rating>
          평점: <span className="star">★</span>{item.vote_average}
        </Rating>
      );
    }
    return null;
  ...
  const renderVideos = () => {
    if (videos.length > 0) {
      const videoKey = videos.find(video => video.site === "YouTube" && video.type === "Trailer")?.key;
      if (videoKey) {
        return (
          <VideoFrame
